...
(() => {
  ...
  getRequest(`${SW_API}/people/1/`)
    .pipe(
      ...
      // PRIMERA RESPUESTA
      // map((val) => val["starships"][0]),
      // switchMap((val) => ajax.getJSON(val))

      // RESPUESTA OFICIAL
      // CON EL ZIP PODEMOS AGRUGAR VALORES
      switchMap((res) => zip(of(res), ajax.getJSON(res["starships"][0]))),
      map(([personajes, naves]) => ({ personajes, naves }))
